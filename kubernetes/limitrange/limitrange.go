...
func ListLimitRange(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	if ns == "" {
		...
		var allItems []v1.LimitRange
		for _, namespace := range namespaces.Items {
			lrs, err := clientset.CoreV1().LimitRanges(namespace.Name).List(ctx, metav1.ListOptions{})
			if err != nil {
				return mcp.NewToolResultText(fmt.Sprintf("Error in listing limitrange in %s: %v", namespace.Name, err)), nil
			}
			allItems = append(allItems, lrs.Items...)
		}
		if outputFmt != "" {
		...
		lrs, err := clientset.CoreV1().LimitRanges(ns).List(ctx, metav1.ListOptions{})
		if err != nil {
			return mcp.NewToolResultText(fmt.Sprintf("Error in listing limitrange in %s: %v", ns, err)), nil
		}
		if outputFmt != "" {
			result, err := outpkg.Format(outputFmt, lrs.Items)
			if err != nil {
				return mcp.NewToolResultText(fmt.Sprintf("Error formatting output: %v", err)), nil
			}
			return mcp.NewToolResultText(result), nil
...
func GetLimitRange(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	lr, err := clientset.CoreV1().LimitRanges(ns).Get(ctx, name, metav1.GetOptions{})
	if err != nil {
		return mcp.NewToolResultText(fmt.Sprintf("Error in getting limitrange %s/%s: %v", ns, name, err)), nil
	}

	if outputFmt != "" {
		result, err := outpkg.Format(outputFmt, lr)
		if err != nil {
			return mcp.NewToolResultText(fmt.Sprintf("Error formatting output: %v", err)), nil
		}
		return mcp.NewToolResultText(result), nil
