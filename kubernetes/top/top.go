...
func TopPod(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	if ns == "" {
		...
		for _, namespace := range namespaces.Items {
			pmList, err := mc.MetricsV1beta1().PodMetricses(namespace.Name).List(ctx, metav1.ListOptions{})
			if err != nil {
				continue
			}
		...
		pmList, err := mc.MetricsV1beta1().PodMetricses(ns).List(ctx, metav1.ListOptions{})
		if err != nil {
...
func TopNode(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	nodesMetrics, err := mc.MetricsV1beta1().NodeMetricses().List(ctx, metav1.ListOptions{})
	if err != nil {
